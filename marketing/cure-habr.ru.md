...
**TL;DR**: `Cure` — это функциональный язык программирования для виртуальной машины BEAM (Erlang/Elixir), который привносит математические доказательства корректности кода прямо во время компиляции. Используя SMT-солверы ([Z3](https://github.com/Z3Prover/z3)/[CVC5](https://github.com/cvc5/cvc5)), `Cure` проверяет типы зависимые от значений, верифицирует конечные автоматы и гарантирует отсутствие целых классов ошибок ещё до запуска программы.
...
И я просто создал [Cure](https://cure-lang.org)
...
FSM (Finite State Machine) — это сердце многих систем на BEAM. Я лично написал ажно целую [библиотеку](https://hexdocs.pm/finitomata) поддержки FSM с «верификацмями» на эликсире, но это, конечно, были костыли, сделанные из палок. В Cure из коробки есть настоящие **верифицируемые конечные автоматы**:
...
Я не пурист, я просто сделал пайпы монадическими. Подробнее [вот тут](https://github.com/am-kantox/cure-lang/blob/main/docs/PIPE_OPERATOR.md).
