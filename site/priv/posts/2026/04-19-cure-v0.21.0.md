...
Two v0.21.0 targets slipped one release forward:
...
- **Remote package-registry index service, publication signing,
  ...
  export path are now scheduled for v0.22.0.
...
Two smaller items also carry forward:
...
- **Full `byte_size` arithmetic through `rest::binary` tails.**
  v0.21.0 binds `rest` to plain `Bitstring`; a future release
  will emit a proper
  `byte_size(rest) == byte_size(scrutinee) - sum_of_preceding_sizes`
