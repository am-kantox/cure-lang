...
test() ->
    ...
    case cure_types:unify(TypeVar, RecursiveType) of
        {ok, _} -> io:format("  PROBLEM: Occurs check failed - infinite type allowed~n");
        {error, Reason3} -> io:format("  GOOD: Occurs check prevented infinite type: ~p~n", [Reason3])
    end,
