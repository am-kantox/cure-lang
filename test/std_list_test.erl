...
test_list_access() ->
    ...
    ?assertEqual([1, 2, 3], test_take([1, 2, 3], 5)),  % Take more than available
    ...
    ?assertEqual([], test_drop([1, 2, 3], 5)),  % Drop more than available
    
    io:format("✓ List access operations test passed~n").

% Helper functions for access
test_nth(List, Index) ->
    lists:nth(Index + 1, List).  % Convert to 1-based for Erlang
...
test_list_predicates() ->
    ...
    % Test all/2
    IsPositive = fun(X) -> X > 0 end,
    ?assertEqual(true, test_all([], IsPositive)),     % Vacuously true
    ...
    % Test any/2
    ?assertEqual(false, test_any([], IsPositive)),    % Empty list
    ?assertEqual(true, test_any([1, -2, -3], IsPositive)),
    ?assertEqual(false, test_any([-1, -2, -3], IsPositive)),
    ?assertEqual(true, test_any([1, 2, 3], IsPositive)),
    
    ?assertEqual(true, test_any([1, 3, 4, 7], IsEven)),   % Contains 4
    ?assertEqual(false, test_any([1, 3, 5, 7], IsEven)),  % All odd
...
test_edge_cases() ->
    ...
    ?assertEqual(500, test_length(LargeEvens)),  % Half should be even
