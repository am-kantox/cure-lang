...
test_compare_function() ->
    ...
    test_compare_values(5, 3, gt),      % 5 > 3 should return Gt
    test_compare_values(3, 5, lt),      % 3 < 5 should return Lt  
    test_compare_values(4, 4, eq),      % 4 == 4 should return Eq
...
test_min_max_clamp() ->
    ...
    ?assertEqual(5, test_clamp(5, 5, 10)),    % At minimum
    ?assertEqual(10, test_clamp(10, 5, 10)),  % At maximum
...
test_utility_functions() ->
    ...
    ?assertEqual(12, Composed(5)),  % (5 + 1) * 2 = 12
    
    % Test flip/2
    Subtract = fun(X, Y) -> X - Y end,
    FlippedSubtract = test_flip(Subtract),
    ?assertEqual(3, FlippedSubtract(5, 8)),  % flip(subtract)(5, 8) = subtract(8, 5) = 8 - 5 = 3
