...
run() ->
    ...
    lists:foreach(fun(#test_result{name = Name, details = Details}) ->
...
test_vector_types() ->
    ...
    try
        ...
        case cure_types:infer_type(VectorLiteral, Env) of
            {ok, Result} ->
                InferredType = element(2, Result),  % Extract type from inference_result
                case cure_types:unify(InferredType, Vec3Type) of
                    ...
                    {error, Reason} ->
                        make_result("Vector types - type unification", false, 
                                   {unify_failed, Reason})
                end;
            {error, Reason} ->
                make_result("Vector types - type inference", false, 
...
test_vector_operations(Env, Vec3Type) ->
    ...
    DotCall = {function_call_expr, 
               {identifier_expr, dot_product, undefined},
               [{identifier_expr, v1, undefined}, {identifier_expr, v2, undefined}],
               undefined},
        ...
        case cure_types:infer_type(ListLiteral, Env) of
            {ok, Result} ->
                ...
                case cure_types:unify(InferredType, List3Type) of
                    ...
                    {error, Reason} ->
                        make_result("Length-indexed lists", false, 
                                   {unify_failed, Reason})
                end;
            {error, Reason} ->
                make_result("Length-indexed lists", false, 
...
test_list_operations(Env, List3Type) ->
    ...
    List2Type = {dependent_type, 'List', [
        #type_param{name = elem_type, value = {primitive_type, 'Int'}},
        #type_param{name = length, value = {literal_expr, 2, undefined}}
    ]},
    
    List5Type = {dependent_type, 'List', [
        #type_param{name = elem_type, value = {primitive_type, 'Int'}},
        #type_param{name = length, value = {binary_op_expr, '+', 
                                            {literal_expr, 3, undefined},
                                            {literal_expr, 2, undefined},
                                            undefined}}
    ]},
    ...
    case cure_types:check_type(undefined, SafeHeadType, Env) of
        ok ->
            case cure_types:check_type(undefined, AppendType, Env) of
                ok ->
                    make_result("Length-indexed lists - operations", true, 
                               safe_operations_valid);
                {error, Reason} ->
                    make_result("Length-indexed lists - operations", false, 
...
test_matrix_dimension_checking() ->
    ...
    try
        ...
        MatrixMultiplyType = {function_type, 
                             [Matrix2x3Type, Matrix3x2Type], 
                             Matrix2x2Type},
        ...
        case cure_types:check_type(undefined, MatrixMultiplyType, Env) of
            ...
            {error, Reason} ->
                make_result("Matrix dimension checking", false, 
...
test_matrix_dimension_errors(Env, Matrix2x3Type) ->
    ...
    InvalidMultiplyType = {function_type, 
                          [Matrix2x3Type, Matrix2x3Type], 
                          {type_var, {id, 1}}}, % Result is unconstrained
...
test_refinement_types() ->
    ...
    try
        ...
        PositiveIntType = {refined_type, {primitive_type, 'Int'}, 
                          fun(X) -> element(2, X) > 0 end},
        ...
        SafeDivideType = {function_type, 
                         [{primitive_type, 'Float'}, NonZeroFloatType],
        ...
        case cure_types:check_type(undefined, SafeDivideType, Env) of
            ok ->
                case cure_types:check_type(undefined, FactorialType, Env) of
                    ...
                    {error, Reason} ->
                        make_result("Refinement types", false, 
...
test_negative_refinement_cases(Env, PositiveIntType) ->
    ...
    case cure_types:check_type(ZeroLiteral, PositiveIntType, Env) of
        {error, _Reason} ->
            make_result("Refinement types", true, 
...
test_phantom_type_safety(Env, MeasurementMetersType, MeasurementFeetType) ->
    ...
    InvalidMixType = {function_type,
                     [MeasurementMetersType, MeasurementFeetType],
                     MeasurementMetersType},
...
test_proof_carrying_code() ->
    ...
    try
        ...
        SortedListType = {proof_type, 'SortedList',
                         {dependent_type, 'List', [
        ...
        InsertSortedType = {function_type,
                           [{type_var, {id, 1}}, SortedListType],
                           SortedListType},
        
        % merge_sorted: (SortedList(T), SortedList(T)) -> SortedList(T)
        MergeSortedType = {function_type,
...
test_liquid_types() ->
    ...
    try
        ...
        ArrayType = {dependent_type, 'Array', [
            #type_param{name = elem_type, value = {type_var, {id, 1}}},
            #type_param{name = length, value = {type_var, {id, 2}}}
        ]},
        
        ArrayIndexType = {liquid_type, 'ArrayIndex',
                         {primitive_type, 'Nat'},
                         [{constraint, '>= 0'}, {constraint, '< length'}],
                         ArrayType},
        
        % safe_array_access: (Array(T, n), ArrayIndex(arr)) -> T
        SafeAccessType = {function_type,
                         [ArrayType, ArrayIndexType],
                         {type_var, {id, 1}}},
        ...
        case cure_types:infer_type(VectorExpr, Env) of
            {ok, Result} ->
                ...
                case has_length_constraint(InferredType, 3) of
                    true ->
                        make_result("Dependent type inference", true,
                                   inferred_basic_structure);
                    false ->
                        make_result("Dependent type inference", false,
...
test_constraint_generation() ->
    ...
    try
        ...
        V1Type = {dependent_type, 'Vector', [
            #type_param{name = elem_type, value = {primitive_type, 'Float'}},
            #type_param{name = length, value = {type_var, {id, 1}}}
        ]},
        
        V2Type = {dependent_type, 'Vector', [
            #type_param{name = elem_type, value = {primitive_type, 'Float'}},
            #type_param{name = length, value = {type_var, {id, 2}}}
        ]},
        ...
        case cure_types:infer_type(CallExpr, EnvWithVecs) of
            {ok, _Result} ->
                ...
                make_result("Constraint generation", true,
                           basic_constraint_generation);
            {error, Reason} ->
                make_result("Constraint generation", false,
                           {call_inference_failed, Reason})
        end
        ...
        case cure_types:solve_constraints(Constraints) of
            {ok, Substitution} ->
                ...
                case maps:size(Substitution) > 0 of
                    true ->
                        make_result("SMT constraint solving", true,
            ...
            {error, Reason} ->
                make_result("SMT constraint solving", false,
        ...
        case cure_types:unify(Vec3Type1, Vec3Type2) of
            {ok, _Subst} ->
                ...
                case cure_types:unify(Vec3Type1, Vec4Type) of
                    {error, _Reason} ->
                        make_result("Unification with dependent types", true,
                                   length_mismatch_detected);
                    {ok, _} ->
                        make_result("Unification with dependent types", false,
...
test_dimension_mismatch_errors() ->
    ...
    try
        ...
        case cure_types:check_type(undefined, InvalidDotType, Env) of
            ...
            ok ->
                make_result("Dimension mismatch errors", false,
...
test_matrix_dimension_errors_comprehensive(Env) ->
    ...
    case cure_types:check_type(undefined, InvalidMatMulType, Env) of
        {error, _Reason} ->
            make_result("Dimension mismatch errors", true,
...
test_refinement_type_violations() ->
    ...
    try
        ...
        case cure_types:check_type(NegativeLiteral, PositiveIntType, Env) of
            {error, _Reason} ->
                make_result("Refinement type violations", true,
...
test_occurs_check_with_dependent_types() ->
    ...
    try
        ...
        RecursiveType = {dependent_type, 'Vector', [
        ...
        case cure_types:unify(TypeVar, RecursiveType) of
            {error, {occurs_check_failed, _, _}} ->
                make_result("Occurs check with dependent types", true,
                           infinite_type_prevented);
            {error, _OtherReason} ->
                make_result("Occurs check with dependent types", true,
                           unification_failed_correctly);
            {ok, _} ->
                make_result("Occurs check with dependent types", false,
...
test_constraint_unsatisfiability() ->
    ...
    try
        ...
        case cure_types:unify(Type1, Type2) of
            {error, _Reason} ->
                make_result("Constraint unsatisfiability", true,
                           conflicting_types_detected);
            {ok, _Subst} ->
                make_result("Constraint unsatisfiability", false,
...
test_nested_dependent_types() ->
    ...
    try
        ...
        case cure_types:check_type(undefined, ProcessMatrixType, Env) of
            ok ->
                make_result("Nested dependent types", true,
                           complex_nesting_valid);
            {error, Reason} ->
                make_result("Nested dependent types", false,
...
test_higher_order_dependent_functions() ->
    ...
    try
        ...
        FoldType = {function_type, [
            {function_type, [{type_var, {id, 1}}, {type_var, {id, 2}}], {type_var, {id, 2}}},
            {type_var, {id, 2}},
            create_vector_type({type_var, {id, 1}}, {type_var, {id, 3}})
        ], {type_var, {id, 2}}},
        ...
        case cure_types:check_type(undefined, MapType, Env) of
            ok ->
                case cure_types:check_type(undefined, FoldType, Env) of
                    ok ->
                        make_result("Higher-order dependent functions", true,
                                   higher_order_types_valid);
                    {error, Reason} ->
                        make_result("Higher-order dependent functions", false,
...
test_dependent_pattern_matching() ->
    ...
    try
        ...
        make_result("Dependent pattern matching", true,
...
test_type_level_computation() ->
    ...
    try
        ...
        List8Type = {dependent_type, 'List', [
            #type_param{name = elem_type, value = {primitive_type, 'Int'}},
            #type_param{name = length, value = {binary_op_expr, '+',
                                               {literal_expr, 3, undefined},
                                               {literal_expr, 5, undefined},
                                               undefined}}
        ]},
        ...
        case cure_types:check_type(undefined, AppendType, Env) of
            ok ->
                make_result("Type-level computation", true,
                           arithmetic_in_types);
            {error, Reason} ->
                make_result("Type-level computation", false,
...
create_matrix_type(Rows, Cols, ElemType) ->
    RowsExpr = case Rows of
        N when is_integer(N) -> {literal_expr, N, undefined};
        _ -> Rows
    end,
    ColsExpr = case Cols of
...
has_length_constraint(Type, ExpectedLength) ->
    case Type of
        ...
        {dependent_type, _, Params} ->
            lists:any(fun(#type_param{name = length, value = {literal_expr, L, _}}) ->
                L =:= ExpectedLength;
            (_) -> false
            end, Params);
        _ -> false
