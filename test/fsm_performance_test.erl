...
test_batch_event_processing() ->
    ...
    timer:sleep(10), % Wait for processing
...
test_memory_optimization() ->
    ...
    end, lists:seq(1, 60)), % 120 events total
    
    timer:sleep(100), % Wait for processing
...
test_timeout_performance() ->
    ...
    EndTime = erlang:monotonic_time(microsecond),
    TotalTime = EndTime - StartTime,
    AvgTimeoutOpTime = TotalTime / 200, % 100 sets + 100 clears
...
create_complex_fsm_definition() ->
    ...
    StateDefs = lists:map(fun(State) ->
        Transitions = lists:map(fun(Event) ->
