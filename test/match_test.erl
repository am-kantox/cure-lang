...
test_complex_clause_bodies() ->
    ...
    #match_expr{
        patterns = [
            #match_clause{
                pattern = #constructor_pattern{name = 'Ok', args = [#identifier_pattern{name = value}]},
                body = #binary_op_expr{op = '+', left = #binary_op_expr{op = '*'}, right = #literal_expr{value = 10}}
            },
            #match_clause{
                pattern = #constructor_pattern{name = 'Error', args = [#identifier_pattern{name = msg}]},
                body = #function_call_expr{function = #identifier_expr{name = length}, args = [#identifier_expr{name = msg}]}
            },
            #match_clause{
...
test_compilation_pipeline() ->
    ...
    case Module of
        {function, FunctionData} ->
            ...
            true = length(Instructions) > 3; % Should have several instructions (label, load_param, make_case, return)
...
test_match_three_or_more_clauses() ->
    ...
    TestCode3 = "def test_three_clauses(x) = match x do\n  1 -> \"one\"\n  2 -> \"two\"\n  _ -> \"other\"\nend",
    ...
    TestCode5 = "def test_five_clauses(value) = match value do\n  Ok(1) -> \"one\"\n  Ok(2) -> \"two\"\n  Error(msg) -> \"error\"\n  None -> \"none\"\n  _ -> \"default\"\nend",
    ...
    #match_clause{pattern = #constructor_pattern{name = 'Ok', args = [#literal_pattern{value = 1}]}} = C1,
    #match_clause{pattern = #constructor_pattern{name = 'Ok', args = [#literal_pattern{value = 2}]}} = C2,
    #match_clause{pattern = #constructor_pattern{name = 'Error', args = [#identifier_pattern{name = msg}]}} = C3,
...
test_parse_match_clause_body_single_expression() ->
    ...
    #match_clause{
        ...
        body = #binary_op_expr{op = '+', left = #identifier_expr{name = x}, right = #literal_expr{value = 5}}
    } = Clause1Simple,
    ...
    TestCodeComplex = "def test_function_call(result) = match result do\n  Ok(value) -> some_func(value)\n  Error(_) -> error_handler()\nend",
    ...
    #match_clause{
        pattern = #constructor_pattern{name = 'Ok', args = [#identifier_pattern{name = value}]},
        body = #function_call_expr{function = #identifier_expr{name = some_func}, args = [#identifier_expr{name = value}]}
    } = ClauseFunc1,
...
test_match_multiple_clauses_no_nested() ->
    ...
    TestCodeFlat = "def test_flat_match(data) = match data do\n  {ok, 1} -> \"first\"\n  {ok, 2} -> \"second\"\n  {error, _} -> \"error\"\n  _ -> \"unknown\"\nend",
    ...
    #match_clause{
        pattern = #tuple_pattern{elements = [#constructor_pattern{name = ok, args = undefined}, #literal_pattern{value = 1}]},
        body = #literal_expr{value = "first"}
    } = FlatClause1,
    
    #match_clause{
        pattern = #tuple_pattern{elements = [#constructor_pattern{name = ok, args = undefined}, #literal_pattern{value = 2}]},
        body = #literal_expr{value = "second"}
    } = FlatClause2,
    
    #match_clause{
        pattern = #tuple_pattern{elements = [#constructor_pattern{name = error, args = undefined}, #wildcard_pattern{}]},
    ...
    TestCodeComplex = "def test_complex_patterns(input) = match input do\n  [head | tail] -> head\n  {value, Ok(result)} -> result\n  {value, Error(msg)} -> msg\n  [] -> \"empty\"\nend",
    ...
    #match_clause{
        pattern = #list_pattern{elements = [#identifier_pattern{name = head}], tail = #identifier_pattern{name = tail}},
        body = #identifier_expr{name = head}
    } = ComplexClause1,
    
    % Verify tuple with constructor patterns
    #match_clause{
        pattern = #tuple_pattern{elements = [#identifier_pattern{name = value}, #constructor_pattern{name = 'Ok', args = [#identifier_pattern{name = result}]}]},
        body = #identifier_expr{name = result}
    } = ComplexClause2,
    
    #match_clause{
        pattern = #tuple_pattern{elements = [#identifier_pattern{name = value}, #constructor_pattern{name = 'Error', args = [#identifier_pattern{name = msg}]}]},
...
test_multi_line_match_clause_bodies() ->
    ...
    TestCodeComplex = "def test_complex_single_line(result) = match result do\n  Ok(value) -> (value + 1) * (value + 2) + (value * 3)\n  Error(_) -> 42 + 24 - 18 * 2\n  _ -> 100\nend",
    ...
    #match_expr{
        patterns = [
            #match_clause{
                pattern = #constructor_pattern{name = 'Ok', args = [#identifier_pattern{name = value}]},
                body = #binary_op_expr{op = '+'}  % Complex nested arithmetic
            },
            #match_clause{
                pattern = #constructor_pattern{name = 'Error', args = [#wildcard_pattern{}]},
                body = #binary_op_expr{}  % Complex arithmetic expression
            },
            #match_clause{
                pattern = #wildcard_pattern{},
    ...
    #match_expr{
        patterns = [
            #match_clause{
                pattern = #tuple_pattern{elements = [#constructor_pattern{name = ok}, #identifier_pattern{name = value}]},
                body = #binary_op_expr{op = '*'} % Complex multiplication expression
            },
            #match_clause{
                pattern = #tuple_pattern{elements = [#constructor_pattern{name = error}, #identifier_pattern{name = code}]},
                body = #if_expr{} % if expression
            },
            #match_clause{
                pattern = #list_pattern{elements = []},
                body = #function_call_expr{function = #identifier_expr{name = length}} % function call
            },
            #match_clause{
                pattern = #wildcard_pattern{},
                body = #function_call_expr{function = #identifier_expr{name = abs}} % function call with negative number
            }
        ]
    } = MatchExprNested,
    ...
    TestCodeFunctionCalls = "def test_function_calls(input) = match input do\n  Some(x) -> max(min(x, 100), 0)\n  None -> some_function(1, 2, 3)\n  _ -> another_func()\nend",
    ...
    #match_expr{
        patterns = [
            #match_clause{
                pattern = #constructor_pattern{name = 'Some', args = [#identifier_pattern{name = x}]},
                body = #function_call_expr{function = #identifier_expr{name = max}} % Nested function calls
            },
            #match_clause{
                pattern = #constructor_pattern{name = 'None', args = undefined},
                body = #function_call_expr{function = #identifier_expr{name = some_function}} % Multi-arg function call
            },
            #match_clause{
                pattern = #wildcard_pattern{},
                body = #function_call_expr{function = #identifier_expr{name = another_func}, args = []} % No-arg function call
            }
        ]
    ...
    #match_expr{
        patterns = [
            #match_clause{
                pattern = #constructor_pattern{name = 'Some', args = [#identifier_pattern{name = x}]},
                body = #if_expr{} % If expression with conditional logic
            },
            #match_clause{
                pattern = #constructor_pattern{name = 'None', args = undefined},
                body = #if_expr{} % If expression
...
contains_nested_match(#match_expr{}) -> true;
...
contains_nested_match(#if_expr{condition = Cond, then_branch = Then, else_branch = Else}) ->
    contains_nested_match(Cond) orelse contains_nested_match(Then) orelse contains_nested_match(Else);
contains_nested_match(#let_expr{bindings = Bindings, body = Body}) ->
    BindingCheck = lists:any(fun(#binding{value = Value}) -> contains_nested_match(Value) end, Bindings),
