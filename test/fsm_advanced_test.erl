    ...
    ActionFSM = #fsm_definition{
        ...
        transitions = #{
            ...
            'Active' => #{
                ...
                cancel => {'Idle',
                    fun(_Data) -> true end,  % Always allow cancel
                    fun(Data) ->
                        History = maps:get(history, Data, []),
...
test_fsm_complex_guards() ->
            ...
            'Processing' => #{
                complete => {'Complete',
                    fun(Data) ->
                        ...
                        CurrentTime = erlang:system_time(millisecond),
                        (CurrentTime - ProcessingStart) >= 50  % At least 50ms processing
                    end,
                    fun(Data) ->
    ...
    {ok, FSMPid1} = cure_fsm_runtime:start_fsm('ComplexGuard', #{
        user_level => 0,  % Too low level
        permissions => []
    }),
    
    cure_fsm_runtime:send_event(FSMPid1, auth),
    timer:sleep(20),
    {ok, 'Init'} = cure_fsm_runtime:get_state(FSMPid1),  % Should remain in Init
...
test_fsm_guard_action_combination() ->
    ...
    DataFlowFSM = #fsm_definition{
        ...
        transitions = #{
            'Ready' => #{
                add_value => {'Accumulating',
                    fun(Data) ->
                        Value = maps:get(input_value, Data, 0),
                        Value > 0  % Guard: only positive values
                    end,
                    fun(Data) ->
                        Value = maps:get(input_value, Data, 0),
            ...
            'Accumulating' => #{
                add_value => {'Accumulating',
                    fun(Data) ->
                        Value = maps:get(input_value, Data, 0),
                        CurrentSum = maps:get(sum, Data, 0),
                        (Value > 0) and ((CurrentSum + Value) < 100)  % Guard: keep under 100
                    end,
                    fun(Data) ->
                        Value = maps:get(input_value, Data, 0),
                        CurrentSum = maps:get(sum, Data, 0),
                ...
                check_threshold => {'Threshold',
                    fun(Data) ->
                        CurrentSum = maps:get(sum, Data, 0),
                        CurrentSum >= 50  % Guard: threshold reached
                    end,
                    fun(Data) ->
...
test_fsm_guard_failure_handling() ->
    ...
    GuardFailureFSM = #fsm_definition{
        ...
        transitions = #{
            'State1' => #{
                strict_transition => {'State2',
                    fun(Data) ->
                        Value = maps:get(value, Data, 0),
                        Value == 42  % Very strict guard
                    end,
                    fun(Data) ->
                ...
                activate => {'Active',
                    ...
                    fun(Data) ->
                        ...
                        Data#{
                            ...
                            activation_history => [erlang:system_time(millisecond) | 
                ...
                process => {'Processed',
                    fun(Data) ->
                        ...
                        try
                            ...
                            _:_ -> false  % Any error in guard evaluation fails the guard
                        end
                    end,
                    fun(Data) ->
                        Data#{processed => true}
            ...
            'Collecting' => #{
                analyze => {'Analyzing',
                    fun(Data) ->
                        ...
                        length(Samples) >= 5  % Need at least 5 samples
                    end,
                    fun(Data) ->
                        Samples = maps:get(samples, Data, []),
                ...
                add_sample => {'Collecting',
                    fun(Data) ->
                        NewSample = maps:get(new_sample, Data, 0),
                        Samples = maps:get(samples, Data, []),
                        (NewSample >= 0) and (length(Samples) < 10)  % Max 10 samples
                    end,
                    fun(Data) ->
                        NewSample = maps:get(new_sample, Data, 0),
                        Samples = maps:get(samples, Data, []),
            ...
            'Analyzing' => #{
                complete => {'Complete',
                    fun(Data) ->
                        ...
                        Average > 10.0  % Complete only if average is high enough
                    end,
                    fun(Data) ->
                        Data#{completion_time => erlang:system_time(millisecond)}
                    end}
