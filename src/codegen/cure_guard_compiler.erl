...
-export([compile_guard/2, compile_guards/2, is_guard_safe/1, optimize_guard/1]).
...
compile_guard_expr(
    #binary_op_expr{
        op = '<>',
        left = #literal_expr{value = Prefix, location = _LocLeft},
...
is_guard_safe(#function_call_expr{
    function = #identifier_expr{name = Fun},
    args = Args
}) ->
...
is_guard_bif(length) ->
    true;
is_guard_bif(string_length) ->
    true;
% Note: <> is handled specially for pattern matching in compile_guard_expr/2
% but is NOT a valid guard BIF for concatenation
% String byte size
is_guard_bif(byte_size) ->
    true;
% Binary pattern matching
is_guard_bif(binary_part) ->
    true;
is_guard_bif(hd) ->
    true;
is_guard_bif(tl) ->
    true;
is_guard_bif(element) ->
    true;
is_guard_bif(is_atom) ->
    true;
is_guard_bif(is_binary) ->
    true;
is_guard_bif(is_boolean) ->
    true;
is_guard_bif(is_float) ->
    true;
is_guard_bif(is_function) ->
    true;
is_guard_bif(is_integer) ->
    true;
is_guard_bif(is_list) ->
    true;
is_guard_bif(is_number) ->
    true;
is_guard_bif(is_pid) ->
    true;
is_guard_bif(is_port) ->
    true;
is_guard_bif(is_record) ->
    true;
is_guard_bif(is_reference) ->
    true;
is_guard_bif(is_tuple) ->
    true;
is_guard_bif(node) ->
    true;
is_guard_bif(self) ->
...
simplify_guard(#binary_op_expr{op = 'andalso', left = #literal_expr{value = false}}) ->
    #literal_expr{value = false, location = undefined};
simplify_guard(#binary_op_expr{op = 'orelse', left = #literal_expr{value = true}}) ->
