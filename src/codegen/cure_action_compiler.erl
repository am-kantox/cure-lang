...
compile_action_expr({emit, Event, Data, Location}, State) ->
    % Compile event data
    {ok, DataInstr, State1} = case Data of
        undefined -> {ok, [], State};
...
compile_action_expr({if_then, Condition, ThenAction, ElseAction, Location}, State) ->
    ...
    {ok, ElseInstr, State5} = case ElseAction of
        undefined -> {ok, [], State4};
...
is_action_value_safe({literal, _Value, _Location}) -> true;
is_action_value_safe({state_var, _Variable, _Location}) -> true;
is_action_value_safe({state_field, _Field, _Location}) -> true;
is_action_value_safe({event_data, _Location}) -> true;
is_action_value_safe({current_state, _Location}) -> true;
...
is_safe_binary_op('++') -> true;  % List concatenation
is_safe_binary_op('--') -> true;  % List subtraction
...
constant_fold_action({if_then, Condition, ThenAction, ElseAction, Location}) ->
    ...
    OptimizedElse = case ElseAction of
        undefined -> undefined;
...
constant_fold_action_value({binary_op, Op, 
                           {literal, L, _}, 
                           {literal, R, _}, 
                           Location}) ->
    try
...
analyze_action_safety_details({call, Function, Args, _Location}, Acc) ->
    FunctionSafety = case is_safe_action_function(Function) of
        true -> safe;
        false -> unsafe
    end,
...
analyze_action_safety_details({if_then, Condition, ThenAction, ElseAction, _Location}, Acc) ->
    ...
    ElseDetails = case ElseAction of
        undefined -> [];
...
analyze_value_safety_details({binary_op, Op, Left, Right, _Location}) ->
    ...
    OpSafety = case is_safe_binary_op(Op) of
        true -> safe;
        false -> unsafe
    end,
...
find_unsafe_reasons({if_then, Condition, ThenAction, ElseAction, _Location}, Acc) ->
    ...
    ElseReasons = case ElseAction of
        undefined -> [];
...
find_unsafe_value_reasons({binary_op, Op, Left, Right, Location}) ->
    ...
    OpReasons = case is_safe_binary_op(Op) of
...
find_unsafe_value_reasons({function_call, Function, Args, Location}) ->
    FunctionReasons = case is_safe_action_function(Function) of
