...
parse_import(State) ->
    ...
    {ModuleToken, State2} = expect(State1, identifier),
    Module = binary_to_atom(get_token_value(ModuleToken), utf8),
    
    {Items, State3} = case match_token(State2, '[') of
        true ->
            {_, State2a} = expect(State2, '['),
            {ItemList, State2b} = parse_atom_list(State2a, []),
    ...
    Location = get_token_location(ModuleToken),
    Import = #import_def{
        module = Module,
        items = Items,
        location = Location
...
parse_if_expression(State) ->
    ...
    {Condition, State2} = parse_expression(State1),
    {_, State3} = expect(State2, then),
    {ThenBranch, State4} = parse_expression(State3),
    {_, State5} = expect(State4, 'else'),
    {ElseBranch, State6} = parse_expression(State5),
