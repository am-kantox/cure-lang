...
-moduledoc "# Cure FSM API Wrapper\n\nThis module provides the Cure-level "
"FSM API that matches the design in lib/std/fsm.cure.\nIt wraps "
"the underlying cure_fsm_runtime gen_server to provide:\n\n- "
"start_fsm/1: Start an FSM instance from a module\n- fsm_cast/2: "
"Send events asynchronously  \n- fsm_advertise/2: Register a "
"name for an FSM\n- fsm_state/1: Query current state and payload\n\nT"
"he FSM definitions in Cure modules are compiled to:\n1. A record "
"type for the payload\n2. A list of transitions with handler "
...
fsm_cast(Target, {EventName, EventPayload}) when
    is_atom(EventName), is_list(EventPayload)
->
...
-spec get_module_fsm_definition(Module :: atom()) ->
    {ok, #fsm_definition{}} | {error, term()}.
