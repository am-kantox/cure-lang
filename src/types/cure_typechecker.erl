...
-export([check_program/1, check_module/1, check_module/2, check_function/1,
         check_function/2, check_expression/2, check_expression/3, infer_type/2, builtin_env/0,
         check_dependent_constraint/3, infer_dependent_type/2, convert_type_to_tuple/1,
         load_stdlib_modules/0, extract_module_functions/1, add_std_function_types/1,
         get_stdlib_function_type/3, create_function_type_from_signature/2,
...
-record(typecheck_error,
        {message :: string(), location :: location(), details :: term()}).
...
check_item({function_def, Name, Params, ReturnType, Constraint, Body, Location}, Env) ->
  ...
  check_function_new({function_def,
                      Name,
                      Params,
                      ReturnType,
                      Constraint,
...
check_item({function_def,
  ...
  check_function_new({function_def,
                      ...
                      Location},
                     Env);
check_item({erlang_function_def,
            Name,
            Params,
            ReturnType,
            Constraint,
            ...
            Location},
           Env) ->
  check_erlang_function_new({erlang_function_def,
                             Name,
                             Params,
                             ReturnType,
                             Constraint,
...
check_module(#module_def{name = Name,
                         exports = Exports,
                         items = Items},
             Env) ->
...
check_function(#function_def{name = Name,
                             params = Params,
  ...
  try
    ...
    case cure_types:infer_type(convert_expr_to_tuple(Body), EnvWithReturnTypeParams) of
      {ok, InferenceResult2} ->
        ...
        case ReturnType of
          undefined ->
            ...
            cure_utils:debug("Adding function ~p to env with inferred type: ~p~n",
                             [Name, FuncType]),
          ...
          _ ->
            ...
            cure_utils:debug("Function ~p - body type: ~p, expected: ~p~n",
            ...
            case cure_types:unify(BodyType, ExpectedReturnType) of
              {ok, _} ->
                ...
                cure_utils:debug("Adding function ~p to env with type: ~p~n", [Name, FuncType]),
                NewEnv = cure_types:extend_env(Env, Name, FuncType),
                {ok, NewEnv, success_result(FuncType)};
              {error, UnifyReason} ->
                ErrorMsg =
                  #typecheck_error{message =
      ...
      {error, InferReason} ->
        ErrorMsg2 =
          #typecheck_error{message = "Failed to infer function body type",
                           location = Location,
    ...
    {ErrorType, Details, ErrorLocation} ->
      ThrownError =
        #typecheck_error{message = format_error_type(ErrorType),
...
check_fsm(#fsm_def{name = Name,
  ...
  case lists:member(Initial, States) of
    false ->
      Error =
        #typecheck_error{message = "Initial state not in states list",
  ...
  ImportEnv =
    case process_imports(Imports, ModuleEnv) of
      {ok, TempEnv} ->
        ...
        case lists:any(fun ({import_def, 'Std', _, _}) ->
                             true;
                           (_) ->
                             false
                       end,
...
check_function_new({function_def, Name, Params, ReturnType, Constraint, Body, Location},
                   Env) ->
  check_function_new({function_def,
                      Name,
                      Params,
    ...
    try
      ...
      case BodyInferenceResult of
        {ok, InferenceResult2} ->
          ...
          case ReturnType of
            undefined ->
              % Function type is inferred
              FuncType = {function_type, ParamTypes, BodyType},
              cure_utils:debug("Adding function ~p (new AST) with inferred type: ~p~n",
                               [Name, FuncType]),
              NewEnv = cure_types:extend_env(Env, Name, FuncType),
              {ok, NewEnv, success_result(FuncType)};
            _ ->
              ...
              case cure_types:unify(BodyType, ExpectedReturnType) of
                {ok, _} ->
                  FuncType = {function_type, ParamTypes, ExpectedReturnType},
                  cure_utils:debug("Adding function ~p (new AST) with explicit type: ~p~n",
                                   [Name, FuncType]),
                  NewEnv = cure_types:extend_env(Env, Name, FuncType),
                  {ok, NewEnv, success_result(FuncType)};
                {error, UnifyReason} ->
        ...
        {error, InferReason} ->
          ...
          cure_utils:debug("  *** Environment size: ~p functions~n",
                           [map_size(case FinalEnv of
                                       #{} ->
                                         FinalEnv;
                                       _ ->
                                         #{}
                                     end)]),
          cure_utils:debug("Failed to infer function ~p body type: ~p~n", [Name, InferReason]),
          ErrorMsg2 =
            #typecheck_error{message = "Failed to infer function body type",
                             location = Location,
                             details = {inference_failed, InferReason}},
      ...
      error:function_clause:Stacktrace ->
        ...
        ErrorMsg3 =
          #typecheck_error{message = "Function clause error during type inference",
                           location = Location,
...
add_stdlib_functions_to_env([], Env) ->
  Env;
add_stdlib_functions_to_env([{{_Module, FunctionName, _Arity}, FunctionType} | Rest],
                            Env) ->
...
check_type_definition(#type_def{name = Name,
                                params = Params,
  ...
  try
    ...
    _:Error ->
      ErrorMsg =
        #typecheck_error{message = "Failed to process type definition",
                         location =
                           #location{line = 0,
                                     column = 0,
                                     file = undefined},
...
process_union_type_variants([Variant | Rest],
                            TypeName,
                            TypeParams,
                            OrigEnv,
                            ConstructorAcc,
  ...
  process_union_type_variants(Rest,
                              TypeName,
                              TypeParams,
                              OrigEnv,
...
extract_module_functions_helper([Item | Rest], Acc) ->
  case Item of
    ...
    {module_def, ModuleName, _Exports, ModuleItems, _Location} ->
      ...
      cure_utils:debug("Found module ~p (no imports) with ~p items~n",
...
check_expression(Expr, Env, ExpectedType) ->
  case cure_types:infer_type(convert_expr_to_tuple(Expr), Env) of
      ...
      case ExpectedType of
        ...
        _ ->
          case cure_types:unify(InferredType, ExpectedType) of
            ...
            {error, Reason} ->
              Error =
                #typecheck_error{message = "Expression type doesn't match expected type",
    ...
    {error, Reason} ->
      Error =
        #typecheck_error{message = "Failed to infer expression type",
                         location = get_expr_location_safe(Expr),
...
process_parameters([#param{name = Name, type = TypeExpr} | RestParams],
                   OrigEnv,
                   TypesAcc,
                   EnvAcc) ->
...
check_state_definitions(StateDefs, States) ->
  ...
  case lists:all(fun(State) -> lists:member(State, States) end, DefinedStates) of
    ...
    false ->
      ...
      {error,
       #typecheck_error{message = "State definitions contain invalid states",
                        location = #location{line = 0, column = 0},
...
convert_expr_to_tuple(#binary_op_expr{op = Op,
                                      left = Left,
                                      right = Right,
                                      location = Location}) ->
  {binary_op_expr, Op, convert_expr_to_tuple(Left), convert_expr_to_tuple(Right), Location};
convert_expr_to_tuple(#function_call_expr{function = Function,
                                          args = Args,
                                          location = Location}) ->
  {function_call_expr,
   convert_expr_to_tuple(Function),
   [convert_expr_to_tuple(Arg) || Arg <- Args],
   Location};
convert_expr_to_tuple(#if_expr{condition = Condition,
                               then_branch = ThenBranch,
                               else_branch = ElseBranch,
                               location = Location}) ->
  {if_expr,
   convert_expr_to_tuple(Condition),
   convert_expr_to_tuple(ThenBranch),
   convert_expr_to_tuple(ElseBranch),
   Location};
convert_expr_to_tuple(#let_expr{bindings = Bindings,
                                body = Body,
                                location = Location}) ->
...
convert_expr_to_tuple(#cons_expr{elements = Elements,
                                 tail = Tail,
                                 location = Location}) ->
...
convert_binding_to_tuple(#binding{pattern = Pattern,
                                  value = Value,
                                  location = Location}) ->
...
convert_pattern_to_tuple(#list_pattern{elements = Elements,
                                       tail = Tail,
                                       location = Location}) ->
...
convert_pattern_to_tuple(#record_pattern{name = Name,
                                         fields = Fields,
                                         location = Location}) ->
...
convert_pattern_to_tuple(#constructor_pattern{name = Name,
                                              args = Args,
                                              location = Location}) ->
  ConvertedArgs =
    case Args of
...
convert_type_to_tuple(#identifier_expr{name = Name})
  when Name =:= 'Float'
       orelse Name =:= 'Int'
       orelse Name =:= 'Bool'
       orelse Name =:= 'String'
...
convert_type_with_env(#union_type{types = Variants}, Env) ->
  ...
  {union_type, ConvertedVariants};
convert_type_with_env(#identifier_expr{name = Name}, _Env)
  when Name =:= 'Float'
       orelse Name =:= 'Int'
       orelse Name =:= 'Bool'
       orelse Name =:= 'String'
    ...
    case Item of
      #function_def{name = Name,
        ...
        try
          ...
          cure_utils:debug("[SIG] Successfully pre-processed ~p with type ~p~n", [Name, FuncType]),
...
error_result(Error) ->
  #typecheck_result{success = false,
                    type = undefined,
                    errors = [Error],
                    warnings = []}.
...
merge_results(Result1 = #typecheck_result{errors = E1, warnings = W1},
              #typecheck_result{success = S2,
                                errors = E2,
                                warnings = W2}) ->
  Result1#typecheck_result{success = Result1#typecheck_result.success andalso S2,
                           errors = E1 ++ E2,
                           warnings = W1 ++ W2}.
...
extract_type_params_helper(#dependent_type{params = Params}, Env) ->
  lists:foldl(fun(#type_param{value = Value}, AccEnv) ->
              ...
              Env,
              Params);
extract_type_params_helper(#function_type{params = ParamTypes, return_type = ReturnType},
                           Env) ->
  % Handle function types - extract type parameters from both parameters and return type
  Env1 =
    lists:foldl(fun(ParamType, AccEnv) -> extract_type_params_helper(ParamType, AccEnv) end,
                Env,
                ParamTypes),
...
extract_type_params_helper({dependent_type, _Name, Params}, Env) ->
  ...
  lists:foldl(fun(Param, AccEnv) -> extract_type_params_helper(Param, AccEnv) end,
              Env,
              Params);
extract_type_params_helper(#list_type{element_type = ElemType, length = LengthExpr},
                           Env) ->
...
extract_type_params_helper_safe(#dependent_type{params = Params}, Env) ->
  lists:foldl(fun(#type_param{value = Value}, AccEnv) ->
              ...
              Env,
              Params);
extract_type_params_helper_safe(#function_type{params = ParamTypes,
                                               return_type = ReturnType},
                                Env) ->
  % Handle function types - extract type parameters from both parameters and return type
  Env1 =
    lists:foldl(fun(ParamType, AccEnv) -> extract_type_params_helper_safe(ParamType, AccEnv)
                end,
                Env,
                ParamTypes),
...
extract_type_params_helper_safe({dependent_type, _Name, Params}, Env) ->
  % Extract type parameters from the parameters list
  lists:foldl(fun(Param, AccEnv) -> extract_type_params_helper_safe(Param, AccEnv) end,
              Env,
              Params);
...
extract_type_params_helper_safe({function_type, ParamTypes, ReturnType}, Env) ->
  ...
  Env1 =
    lists:foldl(fun(ParamType, AccEnv) -> extract_type_params_helper_safe(ParamType, AccEnv)
                ...
                ParamTypes),
  extract_type_params_helper_safe(ReturnType, Env1);
extract_type_params_helper_safe(#list_type{element_type = ElemType, length = LengthExpr},
                                Env) ->
  % Handle list types with length expressions
...
process_parameters_new(Params, Env) ->
  ...
  EnvWithAllTypeParams =
    lists:foldl(fun({param, _Name, TypeExpr, _Location}, AccEnv) ->
...
process_parameters_new([], _OrigEnv, TypesAcc, EnvAcc) ->
  {lists:reverse(TypesAcc), EnvAcc};
process_parameters_new([{param, Name, TypeExpr, _Location} | RestParams],
                       OrigEnv,
                       TypesAcc,
                       EnvAcc) ->
...
import_item_new(Module, {function_import, Name, Arity, _Alias, _Location}, Env) ->
  cure_utils:debug("[IMPORT] Processing function_import ~p/~p from ~p~n",
                   [Name, Arity, Module]),
...
import_item_new(Module, {aliased_import, OriginalName, Alias, _Location}, Env) ->
  ...
  cure_utils:debug("[IMPORT] Processing aliased_import ~p as ~p from ~p~n",
...
import_item_new(Module, Identifier, Env) when is_atom(Identifier) ->
  cure_utils:debug("[IMPORT] Processing atom identifier ~p from ~p~n",
                   [Identifier, Module]),
...
convert_match_clause_to_tuple(#match_clause{pattern = Pattern,
                                            guard = Guard,
                                            body = Body,
                                            location = Location}) ->
  ConvertedGuard =
  ...
  {match_clause,
   convert_pattern_to_tuple(Pattern),
   ConvertedGuard,
   convert_expr_to_tuple(Body),
   Location}.

convert_field_pattern_to_tuple(#field_pattern{name = Name,
                                              pattern = Pattern,
                                              location = Location}) ->
...
convert_constraint_to_smt(Constraint, Env) ->
  case Constraint of
    {binary_op_expr, Op, Left, Right, _Location}
      when Op =:= '>'
           orelse Op =:= '<'
           orelse Op =:= '>='
           orelse Op =:= '=<'
...
add_smt_constraints_to_env(SmtConstraints, Env) ->
  ...
  io:format("Added ~p SMT constraints for when clause verification~n",
